...
Since we have publishing skainet wrapper for Kotlin Notebook into local maven repository, we can now consume by adding depenednecy on the library from localMaven like follows:
...
USE {
    ...
    dependencies {
        implementation("sk.ainet:skainet-notebook:0.0.1")
    }
}
----
